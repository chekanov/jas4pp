/**
 *    Copyright (C)  SCaVis project. The jHPLot package.
					...
					for (int j = 0; j < Ntics; j++) {
						...
						if (t1 >= leftMargin && t1 <= x_end)
							g2.drawLine(t1, y - xRot * subticLength[X], t1, y);
                                         ...
                                         // if X between 1-100, label also subtics in log scale, i.e. 2,3,4,5,6,7,8,9
                                         if (gs.useLogScale(X) && t1 >= leftMargin && t1 <= x_end && j+2<10 && gs.getMinValue(0)>0.5 && gs.getMaxValue(0)<100) {
                                        double ifix = 0;
                                        double ifih = 0;
						...
						if (t1 >= y_end && t1 <= y)
							g2.drawLine(leftMargin, t1, leftMargin + yRot
									* subticLength[Y], t1);
                                         ...
                                         if (gs.useLogScale(Y) && t1 >= y_end && t1 <= y &&  j+2<10 && gs.getMinValue(1)>0.5 && gs.getMaxValue(1)<100) {
                                         double  ifih = 0;
