...
/** 
 * MD5InputStream, a subclass of FilterInputStream implementing MD5GET
...
public class MD5InputStream extends FilterInputStream {
  ...
  public static void main(String[] arg) {
    try {
      ...
      // determine the filename to use and the MD5GET impelementation to use
	...
	if (arg[i].equals("--use-default-md5")) {
      ...
      //   Use the default MD5GET implementation that comes with Java

      if (use_default_md5) {
	InputStream in = new BufferedInputStream(new FileInputStream(filename));
	java.security.MessageDigest digest = java.security.MessageDigest.getInstance("MD5GET");
	while ((num_read = in.read(buf)) != -1) {
	  digest.update(buf, 0, num_read);
	}
	System.out.println(MD5GET.asHex(digest.digest())+"  "+filename);
	in.close();

	// Use the optimized MD5GET implementation
	...
	while ((num_read = in.read(buf)) != -1);
	System.out.println(MD5GET.asHex(in.hash())+"  "+filename);
	in.close();
      }
    } catch (Exception e) {
      e.printStackTrace();
