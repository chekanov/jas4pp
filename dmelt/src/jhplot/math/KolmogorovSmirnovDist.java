...
/**
 * <p>
 * This class computes both the <i>cumulative</i> probability P[D_n <= x] and
 * the <i>complementary cumulative</i> probability P[D_n >= x] of the 2-sided
 * 1-sample Kolmogorov-Smirnov distribution.
 * </p>
 * 
 ...
 * D_n = sup_x |F(x) - S_n(x)|
 * </p>
 * where n is the sample size, S_n(x) is an empirical distribution function, and
 * F(x) is a completely specified theoretical distribution.
 * 
 * 
...
public class KolmogorovSmirnovDist {
	 ...
	 * the Pomeranz algorithms. For n > NEXACT, we use asymptotic methods except
	 * for x close to 0 where we still use the method of Durbin for n <= NKOLMO.
	 * For n > NKOLMO, we use asymptotic methods only and so the precision is
	 * less for x close to 0. We could increase the limit NKOLMO to 10^6 to get
	...
	private static final double[] Factorial = { 1, 1, 2, 6, 24, 120, 720, 5040,
			40320, 362880, 3628800, 39916800, 479001600., 6227020800.,
			87178291200., 1307674368000., 20922789888000., 355687428096000.,
			6402373705728000., 1.21645100408832e+17, 2.43290200817664e+18 };

	// ========================================================================
	// The natural logarithm of factorial n! for 0 <= n <= MFACT

	private static final double[] LnFactorial = { 0., 0., 0.6931471805599453,
			1.791759469228055, 3.178053830347946, 4.787491742782046,
			6.579251212010101, 8.525161361065415, 10.60460290274525,
			12.80182748008147, 15.10441257307552, 17.50230784587389,
			19.98721449566188, 22.55216385312342, 25.19122118273868,
			27.89927138384088, 30.67186010608066, 33.50507345013688,
			36.39544520803305, 39.33988418719949, 42.33561646075348,
			45.3801388984769, 48.47118135183522, 51.60667556776437,
			54.7847293981123, 58.00360522298051, 61.26170176100199,
			64.55753862700632, 67.88974313718154, 71.257038967168,
			74.65823634883016 };
	...
	private static double Pelz(int n, double x) {
		 ...
		 * One-Sample Statistic, Wolfgang Pelz and I. J. Good, Journal of the
		 * Royal Statistical Society, Series B. Vol. 38, No. 2 (1976), pp.
		 * 152-156
		 */
		...
		while (j <= JMAX && Math.abs(term) > EPS * Math.abs(tom)) {
			ti = j + 0.5;
			term = 6 * z6 + 2 * z4 + PI2 * (2 * z4 - 5 * z2) * ti * ti + PI4
					* (1 - 2 * z2) * ti * ti * ti * ti;
			term *= Math.exp(-ti * ti * w);
			tom += term;
			j++;
		...
		while (j <= JMAX && Math.abs(term) > EPS * Math.abs(tom)) {
			...
			term = -30 * z6 - 90 * z6 * z2 + PI2 * (135 * z4 - 96 * z6) * ti
					+ PI4 * (212 * z4 - 60 * z2) * ti * ti + PI2 * PI4 * ti
					* ti * ti * (5 - 30 * z2);
			term *= Math.exp(-ti * w);
			tom += term;
			j++;
	 ...
	 * distribution with sample size n at x. It returns at least 13 decimal
	 * digits of precision for n <= 140, at least 5 decimal digits of precision
	 * for 140 < n <= 100000, and a few correct decimal digits for n > 100000.
	 * 
	 * @param n
	 *            sample size
	 * @param x
	 ...
	 * Kolmogorov-Smirnov distribution with sample size n at x. It returns at
	 * least 10 decimal digits of precision for n <= 140, at least 5 decimal
	 * digits of precision for 140 < n <= 200000, and a few correct decimal
	 * digits for n > 200000.
	 * 
	 * @param n
	 *            sample size
	 ...
	 * random variables sorted into increasing order. See G. Marsaglia, Wai Wan
	 * Tsang and Jingbo Wong, J.Stat.Software, 8, 18, pp 1--4, (2003).
	 */
	...
	private static double DurbinMatrix(int n, double d) {
		...
		// Omit next two lines if you require >7 digit accuracy in the right
		// tail
		if (false) {
			s = d * d * n;
			if (s > 7.24 || (s > 3.76 && n > 99))
				return 1 - 2 * Math
						.exp(-(2.000071 + .331 / Math.sqrt(n) + 1.409 / n) * s);
		}
		k = (int) (n * d) + 1;
		m = 2 * k - 1;
		h = k - n * d;
