...
/**
 * Arrays are faster than object, so this class is here to take full advantage
 * of arrays without encapsulation. All methods are safe, that is, they create
 * copies of arrays whenever necessary This makes for slower methods, but the
 * programmer can always define his own methods optimized for performance.
 * 
...
public final class ArrayMathUtils extends Object {
	 ...
	 */
	// a call to array.clone() may also work although this is a primitive type.
	// I haven't checked
	...
	/**
	 * Renormalizes the array so that its L2 norm is 1 (up to computational
	 * errors).
	 */
	...
	/**
	 * Sets an array to the specified length scraping or padding the beginning
	 * if necessary.
	 */
	...
	/**
	 * Sets an array to the specified length scraping or padding the end if
	 * necessary.
	 */
	...
	/**
	 * Computes the (linear) correlation between two arrays. Squaring this
	 * result and multiply by 100 gives you the percentage of correlation.
	 */
	...
	/**
	 * Computes the median of an array. If the array contains an even number of
	 * elements, then the mean of the lower and upper medians is returned.
	 */
	...
	/*
	 * http://ndevilla.free.fr/median/median/index.html optmed.c
	 * 
	 * @author N. Devillard
	 */
	...
	/**
	 * Sums all components then puts to power n. This is the direct computation
	...
	/**
	 * Fills in with zero to get to the desired length; original array with be
	...
	public static double[] padding(int n0, int pos, double[] v) {
		if ((v.length + pos > n0) || (pos < 0)) {
			throw new IllegalArgumentException("Array is to long for this : "
					+ n0 + ", " + pos + ", " + v.length);
		}
	...
	/**
	 * Adds to an array w, a times v where a is a scalar. Since v can be smaller
	...
	/**
	 * Generates an array going for a to b with steps of size step. If it can't
	 * get to the value b in a finite number of steps, it gets as close as
	 * possible (a can be larger or smaller than b)
	 * 
	 * @param step
	 *            size of steps, must be positive.
	 * @param a
	...
	public static double[] range(double a, double b, double step) {
		if (step <= 0.0) {
			throw new IllegalArgumentException(
					"The argument step should be positive: " + step + " < 0");
		}
		if (a == b) {
	...
	/**
	 * Generates an array going for a to b inclusively with steps of size 1. a
	 * can be smaller or larger than b.
	 */
	...
	/**
	 * produces a double between 0 and 1 based on the given array x and the
	 * index that must be between (0 and x.length) For further information see
	 * also: http://www-ccs.ucsd.edu/matlab/toolbox/nnet/softmax.html
	 */
	...
	/**
	 * Generates an array going for 0 to b with steps of size 1. 0 can be
	...
	/**
	 * Computes the median of an array. If the array contains an even number of
	 * elements, the lower median is returned.
	 */
	...
	/**
	 * Fills in with zeroes to get to the specified length; original array with
	 * be at the specified position
	 * 
	 * @param n0
	 *            length of the new array.
	 * @param pos
	...
	/**
	 * Generates an array going for a to b with steps of size 1. a can be
	 * smaller or larger than b.
	 */
	 ...
	 * 1. (taken out of a code by James Gosling and Kevin A. Smith provided with
	 * Sun's JDK 1.1.7)
	 * 
	 * @param a
	 *            an integer array
	 * @param lo0
	 *            left boundary of array partition (inclusive).
	 * @param hi0
	 *            right boundary of array partition (inclusive).
	 * @deprecated
	 */
