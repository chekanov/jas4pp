 ...
 * @(#)SharpTable.java
 * 
 * $Id: SharpTableModel.java 13895 2011-10-01 00:11:54Z tonyj $
 *
 ...
 */
class SharpTableModel extends DefaultTableModel {
     ...
     */
    public SharpTableModel(JTable table) {
        super();
     ...
     */
    public SharpTableModel(JTable table, int numRows, int numColumns) {
	
        super(numRows, numColumns);

        for(int row = SharpTools.baseRow; row < numRows; row++)
	    for (int col = SharpTools.baseCol; col < numColumns; col++)
     ...
     */
    public SharpTableModel(JTable table, Object[][] data) {

        this(table, data.length + SharpTools.baseRow/*+ 1*/,
...
//    }
	...
	*/
	...
	return toString(new CellRange(SharpTools.baseRow, getRowCount()-1,
				      SharpTools.baseCol, getColumnCount()-1), false, delim);
    }
