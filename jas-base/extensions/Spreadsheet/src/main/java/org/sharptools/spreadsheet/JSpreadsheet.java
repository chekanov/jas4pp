...
{
   /** The base for Row numbering */
   public final static int baseRow = SharpTools.baseRow;

   /** The base for column numbering */
   public final static int baseCol = SharpTools.baseCol;
   ...
   public void setContents(String text, char delim)
   {
      ...
      CellPoint size = SharpTableModel.getSize(text, delim);
      newTableModel(size.getRow(), size.getCol());
      tableModel.fromString(text, delim, 0, 0, new CellRange(SharpTools.baseRow, size.getRow(), SharpTools.baseCol, size.getCol()));
   }
   ...
   public CellPoint find(CellPoint start, String findValue, boolean matchCase, boolean matchCell)
   {
      ...
      return tableModel.look(start, SharpTableModel.fieldParser(findValue), matchCase, matchCell);
   }
   ...
   private void applyColumnWidth(int colWidth)
   {
      if (colWidth > 0)
      {
         for (int i = SharpTools.baseCol; i < table.getModel().getColumnCount(); i++)
         {
   ...
   private CellRange insertRemoveRange(boolean byRow, int start, int end)
   {
      if (byRow)
      {
         return new CellRange(start, end, SharpTools.baseCol, tableModel.getColumnCount() - 1);
      }
      else
      {
         return new CellRange(SharpTools.baseRow, tableModel.getRowCount() - 1, start, end);
      }
   }
   ...
   {
      tableModel = new SharpTableModel(table, rows, cols);
   ...
   private class HeaderMouseAdapter extends MouseAdapter
   {
      public void mouseClicked(MouseEvent e)
      {
         int col = -1;
         int row = -1;
         if (e.getSource() instanceof JTableHeader) col = table.columnAtPoint(e.getPoint());
         if (e.getSource() instanceof JTable) row = table.rowAtPoint(e.getPoint());
         
         int rowCount = table.getRowCount();
         int colCount = table.getColumnCount();

         table.setRowSelectionInterval(SharpTools.baseRow, rowCount - 1);

         if (col >= SharpTools.baseCol) // select column
         {
            table.setColumnSelectionInterval(col,col);
            table.setRowSelectionInterval(SharpTools.baseRow, rowCount - 1);
         }
         else if (row >= SharpTools.baseRow) // select row
         {
            table.setColumnSelectionInterval(SharpTools.baseCol, colCount -1);
   ...
   private static class RowModel implements TableModel
   {
     ...
     */	
   public static String translateColumn(int column) { return Node.translateColumn(column); }
    /**
     ...
     */
   public static String translateRow(int row) { return Node.translateRow(row); }
    /**
     ...
     */	
   public static int translateColumn(String column) { return Node.translateColumn(column); }
    /**
     ...
     */
   public static int translateRow(String row) { return Node.translateRow(row); }
