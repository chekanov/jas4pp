...
public class SimpleEditor extends Plugin implements TextEditorService, FileHandler, XMLPluginIO, PreferencesTopic, WindowFocusListener {
    ...
    public void show(String text, String mimeType, String title) {
        ...
        RTextScrollPane sp = new RTextScrollPane(new EditArea(getApplication(), mimeType, text));
        sp.setFoldIndicatorEnabled(true);
        getApplication().getPageManager().openPage(sp, title, getIconForMimeType(mimeType), makeType(mimeType));
    }
    ...
    public void show(File file, String mimeType) throws IOException {
        ...
        sp.setFoldIndicatorEnabled(true);
        getApplication().getPageManager().openPage(sp, file.getName(), getIconForMimeType(mimeType), makeType(mimeType));
    }
    ...
    private void show(String text, String mimeType, String title, Icon icon) {
        RTextScrollPane sp = new RTextScrollPane( new EditArea(getApplication(), mimeType, text));
        sp.setFoldIndicatorEnabled(true);
        getApplication().getPageManager().openPage(sp, title, icon, makeType(mimeType));
    }
    ...
    private class EditArea extends RSyntaxTextArea implements HasPopupItems, ManagedPage, TextEditor, Scrollable {
        ...
        private void init(Studio app, String mimeType) {
            ...
            if (mimeType.equals("text/python")) setSyntaxEditingStyle(SyntaxConstants.SYNTAX_STYLE_PYTHON);
            if (mimeType.equals("text/java")) setSyntaxEditingStyle(SyntaxConstants.SYNTAX_STYLE_JAVA);
            if (mimeType.equals("text/groovy")) setSyntaxEditingStyle(SyntaxConstants.SYNTAX_STYLE_JAVA);
            //if (mimeType.equals("text/cpp")) setSyntaxEditingStyle(SyntaxConstants.SYNTAX_CPLUSPLUS);
            if (mimeType.equals("text/html")) setSyntaxEditingStyle(SyntaxConstants.SYNTAX_STYLE_HTML);
            if (mimeType.equals("text/xml")) setSyntaxEditingStyle(SyntaxConstants.SYNTAX_STYLE_XML);
            setLineWrap(true);
            ...
            //setFont( SimpleEditor.this.getFont(SimpleEditor.textFields[0]) );
            JPopupMenu popup = new JPopupMenu();
            //JPopupMenu popup = createPopupMenu();
            //appendFoldingMenu(popup);
            JPopupMenu oldpopup = getPopupMenu();
            //for (int j=0; j<5; j++) popup.remove(j); // remove not need built-in menu 
            //System.out.println(oldpopup.getComponentCount());
            // keep folding menu
            JMenuItem foldingMenu = (JMenuItem)(oldpopup.getComponent(10));
            popup.add(foldingMenu);
            popup.addSeparator();
            modifyPopupMenu(popup, app, null); 
            setPopupMenu(popup);
