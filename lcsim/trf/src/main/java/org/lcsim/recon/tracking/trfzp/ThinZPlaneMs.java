...
public class ThinZPlaneMs extends Interactor
{
    ...
    public void interact(ETrack tre)
    {
        ...
        Qxz = (yhat/(norm*zhat))*(yhat/(norm*zhat));
        Qxz += Math.pow((xhat/norm)*(1 + (norm*norm)/(zhat*zhat)),2.0);
        Qxz *= stdTheta;
        Qxz *= stdTheta;
        
        Qyz = (xhat/(norm*zhat))*(xhat/(norm*zhat));
        Qyz += Math.pow((xhat/norm)*(1 + (norm*norm)/(zhat*zhat)),2.0);
        Qyz *= stdTheta;
        Qyz *= stdTheta;
        
        Qxy = -xhat*yhat/(zhat*zhat);
        Qxy += xhat*yhat/(norm*norm)*Math.pow((1 + (norm*norm)/(zhat*zhat)),2.0);
        Qxy *= stdTheta;
        Qxy *= stdTheta;
