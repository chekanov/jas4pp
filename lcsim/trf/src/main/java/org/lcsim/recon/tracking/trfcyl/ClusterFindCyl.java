...
* clusters of type HitCylPhi or HitCylPhiZ indexed by position so
...
public class ClusterFindCyl extends ClusterFindManager
{
     ...
     * @param   stereo The stereo angle of the measurement (0 if axial only measurements).
     * @param   max_chsq_diff The maximum chi-square for a track-hit association.
     */
    public ClusterFindCyl(double radius, double stereo,
    ...
    public int addCluster( Cluster clus )
    {
        ...
        if ( clus.type().equals(ClusCylPhi.staticType()) )
        {
            ClusCylPhi clu = (  ClusCylPhi) clus;
            Assert.assertTrue( clu.surface().equals(_scy) );
            Assert.assertTrue( _stereo == 0.0 );
            keyval = clu.phi();
        }
        else if ( clus.type().equals(ClusCylPhiZ.staticType()) )
        {
            ClusCylPhiZ clu = ( ClusCylPhiZ) clus;
            Assert.assertTrue( clu.surface().equals(_scy) );
            Assert.assertTrue( clu.stereo() == _stereo );
            keyval = clu.phiZ();
        }
        else if ( clus.type().equals(ClusCylPhiZ2D.staticType()) )
        {
...
//            Assert.assertTrue( clu.stereo() == _stereo );
    ...
    public  List clusters(  ETrack tre)
    {
        ...
        double value = tre.vector(0) + _stereo*tre.vector(1);
 ...
 */
   ...
   */
    ...
    public String toString()
    {
        ...
        sb.append( "Stereo angle is " + _stereo + "." + ".\n");
