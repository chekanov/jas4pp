...
/**
 * An LCDD converter for a Silicon tracker model for a fixed target...based off of
 * SiTrackerEndcap2
 * 
 * @author mgraham
 */
public class SiTrackerFixedTargetConverter extends AbstractSubdetectorConverter implements ISubdetectorConverter
{
    ...
    public void convert( Subdetector subdet, Detector detector )
    {
        try
        {
            ...
            try
            {
                ...
                // endcapPosId.setValue(helper.getFieldIndex("barrel"),
                ...
                // endcapNegId.setValue(helper.getFieldIndex("system"),
                // subdet.getSystemID());
                // endcapNegId.setValue(helper.getFieldIndex("barrel"),
                // helper.getEndcapNegativeValue());
                // endcapNeg = new DetectorElement(subdet.getName() + "_negative",
                // subdetDetElem);
            ...
            for ( Iterator i = node.getChildren( "layer" ).iterator(); i.hasNext(); )
            {
                ...
                // layerNegId.setValue(helper.getFieldIndex("system"),
                // subdet.getSystemID());
                // layerNegId.setValue(helper.getFieldIndex("barrel"),
                // helper.getEndcapNegativeValue());
                // layerNegId.setValue(helper.getFieldIndex("layer"), layerId);
                // layerNeg = new DetectorElement(endcapNeg.getName() + "_layer" +
                // layerId, endcapNeg, helper.pack(layerNegId));
                // }
                ...
                for ( Iterator j = layerElement.getChildren( "quadrant" ).iterator(); j.hasNext(); )
                {
                    ...
                    // double dz =
                    // Math.abs(ringElement.getAttribute("dz").getDoubleValue());
                    ...
                    // RotationGeant rot = new RotationGeant(-Math.PI / 2, -Math.PI / 2 -
                    // phi0, 0);
                    // RotationGeant rot = new RotationGeant(0, -Math.PI / 2 - phi0,
                    // -Math.PI / 2);
                    ...
                    // RotationGeant rotr = new RotationGeant(-Math.PI/2, -Math.PI/2 -
                    // phi, Math.PI);

                    // String path2 = "/" + detector.getTrackingVolume().getName() + "/" +
                    // moduleBaseName+"_reflected";
                    // new PhysicalVolume(new Transform3D(pr, rotr),
                    // moduleBaseName+"_reflected", moduleVolume,
                    // detector.getTrackingVolume().getLogicalVolume(), k);
                    // new SiTrackerModule(moduleBaseName+"_reflected", layerNeg, path2,
                    // moduleNumber);
                    // }
