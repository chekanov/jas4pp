...
@version 5 August 1998 $Id: Matrix.java,v 1.1.1.1 2010/11/30 21:31:59 jeremy Exp $
*/
 ...
 * ------------------------ */
   ...
   */
   ...
   @return     absolute value of largest element.
   */
   ...
   public double amax () {
      ...
      for (int i = 0; i < m; ++i) 
	  {
	  	for (int j = 0; j < n; ++j)
	  	{
         if ( Math.abs(A[i][j])>amax ) amax = Math.abs(A[i][j]);
	  	}
      }
      return amax;
   }
   ...
   @return     absolute value of smallest element.
   */

   public double amin() {
      double amin = Math.abs(A[0][0]);
      for (int i = 0; i < m; ++i) 
	  {
	  	for (int j = 0; j < n; ++j)
	  	{
         if ( Math.abs(A[i][j])<amin ) amin = Math.abs(A[i][j]);
	  	}
      }
      return amin;
   }   
